...
def _fit_power_law(logT: List[float], logC: List[float], logr: List[float]) -> Dict[str, float]:
    n = len(logr)
    X = np.vstack([np.ones(n), np.array(logT), np.array(logC)]).T
    y = np.array(logr)
    coeff, *_ = np.linalg.lstsq(X, y, rcond=None)
    b0, a, b = [float(x) for x in coeff.tolist()]
    pred = X @ coeff
    ss_res = float(np.sum((y - pred) ** 2))
    ss_tot = float(np.sum((y - float(np.mean(y))) ** 2))
    r2 = 1.0 - (ss_res / ss_tot if ss_tot > 0 else 0.0)
    return {"alpha_hat": a, "beta_hat": b, "logk_hat": b0, "r2": r2}


def run(spec: SpecPhaseSurfaceV1, out_dir: Path) -> Dict[str, Any]:
    _set_det(spec.seed)
    ...
    boundary_surface: List[Dict[str, Any]] = []
    hazard_surface: List[Dict[str, Any]] = []
    ...
    for T in spec.temps:
        ...
        for C in spec.context_fracs:
            ...
            hazard_r_at: Optional[float] = None
            ...
            for delta in spec.deltas:
                ...
                cell_seed = int(_sha256_hex(f"{spec.seed}|{t}|{c}|{d}")[:8], 16)
                ...
                if abs(d - float(spec.hazard_delta)) < 1e-12:
                    hazard_r_at = float(hazard_r)
            ...
            boundary_surface.append(
                {"T": t, "C": c, "C_target_prompt_len": int(ctx_len_by_C[c]), "tau": spec.tau_survive, "delta_c": delta_c}
            )

            if hazard_r_at is None:
                hazard_r_at = float("nan")
            hazard_surface.append({"T": t, "C": c, "delta": float(spec.hazard_delta), "hazard_r": float(hazard_r_at)})

    logT: List[float] = []
    logC: List[float] = []
    logr: List[float] = []
    for row in hazard_surface:
        r = float(row["hazard_r"])
        T = float(row["T"])
        C = float(row["C"])
        if math.isfinite(r) and r > 0.0 and T > 0.0 and C > 0.0:
            logT.append(math.log(T))
            logC.append(math.log(C))
            logr.append(math.log(r))

    fit = _fit_power_law(logT, logC, logr) if len(logr) >= 6 else {"alpha_hat": float("nan"), "beta_hat": float("nan"), "logk_hat": float("nan"), "r2": 0.0}
    ...
    boundary_sha = _sha256_hex(_canon(boundary_surface))
    hazard_sha = _sha256_hex(_canon(hazard_surface))
    fit_sha = _sha256_hex(_canon(fit))

    _write_json(out_dir / "spec.json", spec_obj)
    _write_json(out_dir / "grid.json", grid)
    _write_json(out_dir / "boundary_surface.json", boundary_surface)
    _write_json(out_dir / "hazard_surface.json", hazard_surface)
    _write_json(out_dir / "interaction_fit.json", fit)
    ...
    manifest = {
        ...
        "boundary_surface_sha256": boundary_sha,
        "hazard_surface_sha256": hazard_sha,
        "interaction_fit_sha256": fit_sha,
        "status": "COMPLETE",
    }
...
def main() -> None:
    print("[RUN] run_phase_surface_v1", flush=True)
    ...
    ap.add_argument("--temps", default="0.5,0.7,0.9,1.1,1.3")
    ...
    ap.add_argument("--hazard-delta", type=float, default=0.0)
    ...
    ap.add_argument("--out", default="out/phase_surface_v1")
    args = ap.parse_args()
    ...
    spec = SpecPhaseSurfaceV1(
        exp_id="phase_surface_v1",
        exp_version="v1",
        ...
        hazard_delta=float(args.hazard_delta),
    )
    run(spec, out_dir=Path(args.out))
    print_write_report("PHASE_SURFACE", Path(args.out), [
        "spec.json",
        "grid.json",
        "hazard_surface.json",
        "boundary_surface.json",
        "interaction_fit.json",
        "manifest.json",
    ])
